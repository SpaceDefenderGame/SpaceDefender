...
SpaceDefender.Enemy.prototype.damage = function(amount) {
  ...
  if(this.health <= 0) {
	...
	setTimeout ((function() {SpaceDefender.GameState.scoreLabel.x = SpaceDefender.GameState.game.world.width - SpaceDefender.GameState.game.width / 12 - SpaceDefender.GameState.scoreText.width - 20;}).bind(this), 0);
    this.throwCargo();
...
SpaceDefender.Enemy.prototype.shoot = function() {
  if (Math.abs(SpaceDefender.GameState.player.x / this.game.world.width - this.x / this.game.world.width) < 0.15*this.difficultyCoefficient) {
  let bullet = this.enemyBullets.getFirstExists(false);
  if(!bullet) {
    bullet = new SpaceDefender.EnemyBullet(this.game, this.x, this.bottom);
    ...
    let bulletDirection = this.game.physics.arcade.moveToObject(bullet, SpaceDefender.GameState.player, 350*this.difficultyCoefficient);
...
SpaceDefender.Enemy.prototype.rotate = function() {
  if (Math.abs(SpaceDefender.GameState.player.x / this.game.world.width - this.x / this.game.world.width) < 0.2*this.difficultyCoefficient) { 
