...
SpaceDefender.GameState = {
  ...
  preload: function() {
	this.load.audio('orchestra', ['assets/audio/battlemusic'+this.currentLevel+'.mp3', 'assets/audio/battlemusic'+this.currentLevel+'.ogg']);
	...
	this.load.spritesheet('lightBlueEnemy', 'assets/images/enemies/light-blue-enemy.png', 50, 44, 3, 0, 0);
	this.load.spritesheet('redBlackEnemy', 'assets/images/enemies/red-black-enemy.png', 50, 87, 3, 0, 0);
	...
	this.load.audio('mission-failed', ['assets/audio/mission-failed.mp3', 'assets/audio/mission-failed.ogg']);
  ...
  create: function() {
    ...
    this.initBullets();
    this.shootingTimer = this.game.time.events.loop(Phaser.Timer.SECOND/this.bulletFrequency, this.createPlayerBullet, this);
	...
	//player
    this.player = this.game.add.sprite(this.game.world.centerX, this.game.world.height - 100, 'player', 0);
  ...
  damagePlayer: function(player, bullet) {
	...
	setTimeout((function() {this.player.animations.play('fly')}).bind(this), 300);
	...
	switch(cargo.cargoType) {
		case 'medicine':
		  if (this.player.health < SpaceDefender.maxHealth) {
			  ...
			  this.player.health > SpaceDefender.maxHealth ? this.player.health = SpaceDefender.maxHealth : this.player.health = this.player.health;
		      this.lifeText.text = this.player.health;
              cargo.kill();              
        ...
        case 'fastFire':
		  if (this.shootingTimer.delay > Phaser.Timer.SECOND/(this.bulletFrequency*1.5)+1) {
			  cargo.kill();
  ...
  killPlayer: function() {
    ...
    this.orchestra.stop();	
    setTimeout((function(){this.game.state.start('LevelFailed', true, false, this.currentLevel)}).bind(this), 2000);
  },
  
  createEnemy: function(x, y, health, key, scale, speedX, speedY, strength){    
    let enemy = new SpaceDefender.Enemy(this.game, x, y, key, health, this.enemyBullets, this.cargoes, strength);
  ...
  callBoss: function() {
	  ...
	  this.createEnemy(bossEnemy.x * this.game.world.width, -50, bossEnemy.health, bossEnemy.key, bossEnemy.scale, bossEnemy.speedX, bossEnemy.speedY, bossEnemy.strength);
  },
  ...
  scheduleNextEnemy: function() {
	...
	nextEnemy.speedX = this.game.rnd.pick([-1, 1])*this.game.rnd.integerInRange(50, 100)*this.game.difficultyCoefficient;
    ...
    nextEnemy.x = this.game.rnd.integerInRange(2, 8) / 10;
      ...
      this.nextEnemyTimer = this.game.time.events.add(nextTime, function(){
        this.createEnemy(nextEnemy.x * this.game.world.width, -50, nextEnemy.health, nextEnemy.key, nextEnemy.scale, nextEnemy.speedX, nextEnemy.speedY);
